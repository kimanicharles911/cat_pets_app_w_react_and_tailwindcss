...
const ScottishComponent = () => {
  ...
  const [likeClick, setLikeClick] = useState(() => {
    const savedLikeState = localStorage.getItem("ScottishLikes");
    return savedLikeState ? JSON.parse(savedLikeState) : false;
  });
  const [dislikeClick, setDislikeClick] = useState(() => {
    const savedDislikeState = localStorage.getItem("ScottishDislikes");
  ...
  useEffect(() => {
    localStorage.setItem("ScottishLikes", JSON.stringify(likeClick));
    localStorage.setItem("ScottishDislikes", JSON.stringify(dislikeClick));
  }, [likeClick, dislikeClick]);

  const handleFetch = () => {
    fetch(`https://api.thecatapi.com/v1/images/search?breed_id=sfol`, {
      method: "GET",
      headers: {

...
const AmericanComponent = () => {
  ...
  const [likeClick, setLikeClick] = useState(() => {
    const savedLikeState = localStorage.getItem("AmericanLikes");
    return savedLikeState ? JSON.parse(savedLikeState) : false;
  });
  const [dislikeClick, setDislikeClick] = useState(() => {
    const savedDislikeState = localStorage.getItem("AmericanDislikes");
  ...
  useEffect(() => {
    localStorage.setItem("AmericanLikes", JSON.stringify(likeClick));
    localStorage.setItem("AmericanDislikes", JSON.stringify(dislikeClick));
  }, [likeClick, dislikeClick]);

  const handleFetch = () => {
    fetch(`https://api.thecatapi.com/v1/images/search?breed_id=abob`, {
      method: "GET",
      headers: {
...
export default AmericanComponent;

...
const PersianComponent = () => {
  ...
  const [likeClick, setLikeClick] = useState(() => {
    const savedLikeState = localStorage.getItem("PersianLikes");
    return savedLikeState ? JSON.parse(savedLikeState) : false;
  });
  const [dislikeClick, setDislikeClick] = useState(() => {
    const savedDislikeState = localStorage.getItem("PersianDislikes");
  ...
  useEffect(() => {
    localStorage.setItem("PersianLikes", JSON.stringify(likeClick));
    localStorage.setItem("PersianDislikes", JSON.stringify(dislikeClick));
  }, [likeClick, dislikeClick]);

  const handleFetch = () => {
    fetch(`https://api.thecatapi.com/v1/images/search?breed_id=pers`, {
      method: "GET",
      headers: {
...
export default PersianComponent;

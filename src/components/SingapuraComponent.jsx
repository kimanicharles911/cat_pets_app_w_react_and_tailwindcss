...
const SingapuraComponent = () => {
  ...
  const [likeClick, setLikeClick] = useState(() => {
    const savedLikeState = localStorage.getItem("SingapuraLikes");
    return savedLikeState ? JSON.parse(savedLikeState) : false;
  });
  const [dislikeClick, setDislikeClick] = useState(() => {
    const savedDislikeState = localStorage.getItem("SingapuraDislikes");
  ...
  useEffect(() => {
    localStorage.setItem("SingapuraLikes", JSON.stringify(likeClick));
    localStorage.setItem("SingapuraDislikes", JSON.stringify(dislikeClick));
  }, [likeClick, dislikeClick]);

  const handleFetch = () => {
    fetch(`https://api.thecatapi.com/v1/images/search?breed_id=sing`, {
      method: "GET",
      headers: {
...
export default SingapuraComponent;

...
const RagaMuffinComponent = () => {
  ...
  const [likeClick, setLikeClick] = useState(() => {
    const savedLikeState = localStorage.getItem("RagaMuffinLikes");
    return savedLikeState ? JSON.parse(savedLikeState) : false;
  });
  const [dislikeClick, setDislikeClick] = useState(() => {
    const savedDislikeState = localStorage.getItem("RagaMuffinDislikes");
  ...
  useEffect(() => {
    localStorage.setItem("RagaMuffinLikes", JSON.stringify(likeClick));
    localStorage.setItem("RagaMuffinDislikes", JSON.stringify(dislikeClick));
  }, [likeClick, dislikeClick]);

  const handleFetch = () => {
    fetch(`https://api.thecatapi.com/v1/images/search?breed_id=raga`, {
      method: "GET",
      headers: {
...
export default RagaMuffinComponent;
